...
static void suspend_worker(thread_info_t *info)
{
        ...
        int quanta_remaining = 0; //intialize value
	...
	quanta_remaining = info->quanta;

	/* TODO: decide whether to cancel or suspend thread */
	if(quanta_remaining > 0) {
	  /*
...
void setup_sig_handlers() {
	...
	struct sigaction action1, action2, action3; //action1 for SIGALRM, action2 for SIGTERM, action3 for SIGUSR1

	action1.sa_flags = 0;
	action2.sa_flags = 0;
	action3.sa_flags = 0;

	action1.sa_handler = timer_handler;
	action2.sa_handler = cancel_thread;
	action3.sa_handler = suspend_thread;
...
static void *scheduler_run(void *unused)
{
	...
	timerspecs.it_value.tv_sec = QUANTUM;
	timerspecs.it_value.tv_nsec = 0;
	timerspecs.it_interval.tv_sec = QUANTUM;
	timerspecs.it_interval.tv_nsec = 0;

	timer_settime(timer, 0, &timerspecs, NULL);
